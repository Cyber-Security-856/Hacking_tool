...
{c2}Please Enter The Target IP Or Site Address'''.format(
    c1=Q(),
    c2=Q()))
    site = input(Q()+'=> ')
    print('\n\n')
...
{c2} Enter The Target IP Website Address'''.format(
    c1=Q(),
    c2=Q()))
    target = input(Q()+'=> ')
    print('\n\n')
...
{c2} Enter The Target IP Or Website Address'''.format(
    c1=Q(),
    c2=Q()))
    site = input(Q()+'=> ')
    ip = getip(rmhttp(site))
...
while True:
    Clear()
    Print_Logo()
    op = input(Q()+'''┌─[CyberSecurity@MKDIR]
└──╼ $ ''')
    if op == '1':
        Clear()
        whois()
